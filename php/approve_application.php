...
$query_update = "UPDATE applications SET status = 'Approved' WHERE id = ?";
...
if ($stmt_update->execute()) {
    ...
    if ($details) {
        ...
        $student_message = "Your application (ID: $application_id) has been approved on $current_date.";
        ...
        if ($teacher) {
            ...
            $teacher_message = "Application with ID $application_id has been approved. Assist the student with the exam date.";
    ...
    $message = "Failed to approve application: " . $stmt_update->error;
}
$stmt_update->close();
